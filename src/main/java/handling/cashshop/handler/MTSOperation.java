...
public class MTSOperation {
    ...
    public static void MTSOperation(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (op == 2) { //put up for sale
            ...
            if (invType != 1 && invType != 2) { //pet?
                c.announce(MTSCSPacket.INSTANCE.getMTSFailSell());
                doMTSPackets(cart, c);
                return;
            }
            ...
            if (type == MapleInventoryType.EQUIP) {
                ...
                if (eq.getState(false) > 0 || eq.getEnhance() > 0 || eq.getDurability() > -1) {
                    c.announce(MTSCSPacket.INSTANCE.getMTSFailSell());
                    doMTSPackets(cart, c);
                    return;
                }
            ...
            c.announce(MTSCSPacket.INSTANCE.getMTSConfirmSell());
            ...
            if (!MTSStorage.getInstance().removeFromBuyNow(slea.readInt(), c.getPlayer().getId(), true)) {
                c.announce(MTSCSPacket.INSTANCE.getMTSFailCancel());
            } else {
                c.announce(MTSCSPacket.INSTANCE.getMTSConfirmCancel());
            ...
            if (item != null && item.getQuantity() > 0 && MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                ...
                if (pos >= 0) {
                    ...
                    c.announce(MTSCSPacket.INSTANCE.getMTSConfirmTransfer(ItemConstants.getInventoryType(item_.getItemId()).getType(), pos)); //IF this is actually pos and pos
                    sendMTSPackets(cart, c, true);
                    return;
                } else {
                    //System.out.println("addByItem is less than 0");
                    c.announce(MTSCSPacket.INSTANCE.getMTSFailBuy());
                }
            } else {
            ...
            if (MTSStorage.getInstance().checkCart(id, c.getPlayer().getId()) && cart.addToCart(id)) {
                c.announce(MTSCSPacket.INSTANCE.addToCartMessage(false, false));
            } else {
                c.announce(MTSCSPacket.INSTANCE.addToCartMessage(true, false));
            }
        } else if (op == 10) { //delete from cart
            final int id = slea.readInt();
            if (cart.getCart().contains(id)) {
                cart.removeFromCart(id);
                c.announce(MTSCSPacket.INSTANCE.addToCartMessage(false, true));
            } else {
                c.announce(MTSCSPacket.INSTANCE.addToCartMessage(true, true));
            }
            ...
            if (mts != null && mts.getCharacterId() != c.getPlayer().getId()) {
                if (c.getPlayer().getCSPoints(1) > mts.getRealPrice()) {
                    if (MTSStorage.getInstance().removeFromBuyNow(mts.getId(), c.getPlayer().getId(), false)) {
                        ...
                        c.announce(MTSCSPacket.INSTANCE.getMTSConfirmBuy());
                        sendMTSPackets(cart, c, true);
                        return;
                    } else {
                        c.announce(MTSCSPacket.INSTANCE.getMTSFailBuy());
    ...
    public static void MTSUpdate(final MTSCart cart, final MapleClient c) {
        ...
        c.announce(MTSCSPacket.INSTANCE.getMTSWantedListingOver(0, 0));
        doMTSPackets(cart, c);
    }
    ...
    private static void sendMTSPackets(final MTSCart cart, final MapleClient c, final boolean changed) {
        ...
        c.announce(MTSCSPacket.INSTANCE.showMTSCash(c.getPlayer()));
